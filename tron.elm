...
data GameState = Ended String Space | Playing PlayerState PlayerState Paused Space Enter
...
tailIncrement = 8/frameRate -- amount by which tail grows every second
tailL = toFloat 256
...
initialGameState =
    ...
    in Playing bike1 bike2 False True False

showGameState : GameState -> Element
showGameState gs = 
    let forms = [filled black (rect width height)] ++ elements
        elements = case gs of
                     (Ended m _ ) -> [toForm (centered (typeface ["arial"] (Text.height 20 (Text.color yellow (toText (join "" [m, "\n\n\n", (wrap " Press space to start "), "\n", (wrap "Press enter to pause")]))))))]
                     (Playing (PlayerState pos1 o1 _ _ tail1) (PlayerState pos2 o2 _ _ tail2) paused _ _) ->
...
step (Input (KeybInput arrows wasd space enter) _) gs =
    case gs of
        Ended m oldSpace -> if space /= oldSpace && space then initialGameState else Ended m space
        Playing (PlayerState pos1 o1 v1 tailL1 tail1) (PlayerState pos2 o2 v2 tailL2 tail2) paused oldSpace oldEnter ->
            if space /= oldSpace && space then initialGameState else
                if enter /= oldEnter && enter then Playing (PlayerState pos1 o1 v1 tailL1 tail1) (PlayerState pos2 o2 v2 tailL2 tail2) (not paused) space enter else
                    if paused then Playing (PlayerState pos1 o1 v1 tailL1 tail1) (PlayerState pos2 o2 v2 tailL2 tail2) paused space enter else
                        ...
                        in
                            if dead1 && dead2 then Ended "It's a tie!" space else
                                if dead1 then Ended "Player 2 wins!" space else
                                    if dead2 then Ended "Player 1 wins!" space else
                                        Playing (PlayerState new_pos1 new_o1 v1 new_tailL1 new_tail1) (PlayerState new_pos2 new_o2 v2 new_tailL2 new_tail2) paused space enter
